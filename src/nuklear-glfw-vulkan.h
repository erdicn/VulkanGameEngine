...
static char* guiReadFile(const char* filename, size_t* out_size){
...
void prepare_pipeline(struct nk_vulkan_adapter *adapter) {
  ...
  size_t nuklearshaders_nuklear_vert_spv_len = 0;
  char* nuklearshaders_nuklear_vert_spv = guiReadFile("nuklearvert.spv", &nuklearshaders_nuklear_vert_spv_len);
  size_t nuklearshaders_nuklear_frag_spv_len = 0;
  char* nuklearshaders_nuklear_frag_spv = guiReadFile("nuklearfrag.spv", &nuklearshaders_nuklear_frag_spv_len);
  VkPipelineShaderStageCreateInfo shader_stages[2] = {
      create_shader(adapter, nuklearshaders_nuklear_vert_spv,
                    (uint32_t)nuklearshaders_nuklear_vert_spv_len,
                    VK_SHADER_STAGE_VERTEX_BIT),
      create_shader(adapter, nuklearshaders_nuklear_frag_spv,
                    (uint32_t)nuklearshaders_nuklear_frag_spv_len,
                    VK_SHADER_STAGE_FRAGMENT_BIT)};
