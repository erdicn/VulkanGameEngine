...
void init_overlay(AppVariables_t* app) {
    // 1. Init the UI Context
    ctx = nk_glfw3_init(app->window, app->device, app->physical_device, app->graphics_queue_family_index, app->swap_chain.image_views, app->swap_chain.image_views_len, app->swap_chain.extend.width, app->swap_chain.extend.height, app->swap_chain.image_format, NK_GLFW3_INSTALL_CALLBACKS);
  ...
  // /* Load Cursor: if you uncomment cursor loading please hide the cursor */
  {
    ...
    nk_glfw3_font_stash_end(app->graphics_queue);
...
VkSemaphore submit_overlay(AppVariables_t* app,struct overlay_settings *settings,
                           VkQueue graphics_queue, uint32_t buffer_index,
                           VkSemaphore wait_semaphore) {
  struct nk_colorf background = {.r = settings->bg_color[0], .g = settings->bg_color[1],
                                 .b = settings->bg_color[2], .a = settings->bg_color[3]};
