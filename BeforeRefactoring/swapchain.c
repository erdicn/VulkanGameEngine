...
SwapChainSupportDetails_t querySwapChainSupport(VkPhysicalDevice device, AppVariables_t* app){
    ...
    if (details.formats_len != 0) {
        details.formats = realloc(details.formats, sizeof(VkSurfaceFormatKHR)*details.formats_len);
    ...
    if (details.present_modes_len != 0) {
        details.presentModes = realloc(details.presentModes, sizeof(VkPresentModeKHR)*details.present_modes_len);
...
void createSwapChain(AppVariables_t* app) {
    ...
    app->swap_chain.images = realloc(app->swap_chain.images, imageCount*sizeof(VkImage)); 
...
void createImageViews(AppVariables_t* app) {
    ...
    app->swap_chain.image_views = realloc(app->swap_chain.image_views, sizeof(VkImageView)*app->swap_chain.image_views_len);
...
void createFramebuffers(AppVariables_t* app){
    ...
    app->swap_chain.frame_buffers = realloc(app->swap_chain.frame_buffers, sizeof(VkFramebuffer)*app->swap_chain.frame_buffers_len);
...
void createOverlayImages(AppVariables_t* app)
{
    ...
    app->overlay.images = realloc(app->overlay.images, app->overlay.images_len*sizeof(VkImage));
    app->overlay.image_memories_len = app->swap_chain.images_len;
    app->overlay.image_memories = realloc(app->overlay.image_memories, app->overlay.image_memories_len*sizeof(VkDeviceMemory));
    for (size_t i = 0; i < app->overlay.images_len; i++)
    {
...
void createOverlayImageViews(AppVariables_t* app)
{
    ...
    app->overlay.image_views = realloc(app->overlay.image_views, app->overlay.image_views_len*sizeof(VkImageView));
